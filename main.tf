...
resource "aws_security_group" "tf-sec-gr" {
  ...
  ingress { #its for educational. normally db shouldn't be reachable from "anywhere"
    from_port   = 5432
    protocol    = "tcp"
    to_port     = 5432
    cidr_blocks = ["0.0.0.0/0"]
...
output "node_public_ip" { #nodejs public ip
  value = aws_instance.managed_nodes[1].public_ip

}

output "postgre_private_ip" { #database private ip
